...
class Rustup {
  ...
  static List<_Toolchain> _getInstalledToolchains() {
    ...
    // Include Rust's named channels and pinned semver toolchains (for example
    // `1.97.0-x86_64-unknown-linux-gnu`), but continue to ignore unrelated
    // custom toolchains that may not support `rustup target list`.
    final discoverableToolchain =
        RegExp(r'^(stable|beta|nightly|[0-9]+\.[0-9]+(?:\.[0-9]+)?)(?:-|\s|$)');
    final lines = res.stdout
        .toString()
        .split('\n')
        .where((e) => e.isNotEmpty && e.startsWith(discoverableToolchain))
